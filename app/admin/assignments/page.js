  ...
  async function downloadStageZip(stage) {
    ...
    try {
      ...
      const subs = submissions.filter(
        (s) => s.draft_stage === stage && s.storage_path,
      );
      for (const sub of subs) {
        ...
        const name =
          `${student.name}_${selected.title}_${stage}.${ext}`.replace(
            /\s+/g,
            "_",
          );
        zip.file(name, blob);
    ...
    try {
      ...
      for (const student of students) {
        ...
        const matchingFile = Object.keys(zip.files).find(
          (f) =>
            f.toLowerCase().includes(namePart.toLowerCase()) &&
            !zip.files[f].dir,
        );
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              {selected.draft_stages.map((stage) =>
                selected.due_dates[stage] ? (
                  <div
                    key={stage}
                    className="text-xs font-bold tracking-widest uppercase border border-black px-3 py-2"
                  >
                    {stage}: Due{" "}
                    {new Date(selected.due_dates[stage]).toLocaleDateString(
                      "en-US",
                        ...
                        <div className="flex gap-2 mt-2">
                          ...
                          <label className="text-xs font-bold tracking-widest border border-black px-2 py-1 hover:bg-black hover:text-white transition-colors cursor-pointer normal-case">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.files[0])
                                  handleBulkReturn(stage, e.target.files[0]);
                              }}
                            />
                          </label>
                        </div>
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <label className="cursor-pointer border border-black px-2 py-1 hover:bg-black hover:text-white transition-colors">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        if (e.target.files[0])
                                          handleReturn(sub, e.target.files[0]);
                                      }}
                                    />
                                  </label>
                                </div>
      ...
      <main className="p-8">
        ...
        {showForm && (
          <form
            ...
            className="border border-black p-6 mb-8 flex flex-col gap-4"
          >
            <input
              type="text"
              placeholder="Title"
