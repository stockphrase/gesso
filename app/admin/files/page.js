...
export default function AdminFilesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // For now use first course — we will wire up multi-course later
  ...
  async function handleUpload(e) {
    ...
    const fileInput = e.target.elements.zipfile
                ...
                <div key={folder.id}>
                  ...
                  <div className="py-4 flex items-center justify-between gap-4">
                    <button
                      ...
                      className="text-sm font-bold text-black hover:underline text-left"
                    >
                      {expanded === folder.id ? '▼' : '▶'} {folder.name}
                    </button>
                    ...
                    <div className="pb-4 pl-4 flex flex-col gap-2">
                      ...
                      {!folderFiles[folder.id] ? (
                        ...
                        folderFiles[folder.id].map((file) => (
                          <div
                            key={file.id}
                            className="flex items-center justify-between text-sm border border-gray-100 p-2"
                          >
