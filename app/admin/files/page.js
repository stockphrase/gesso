...
export default function AdminFilesPage() {
  ...
  async function loadFolders(cId) {
    const { data } = await supabase
      .from('file_folders').select('*').eq('course_id', cId).order('name')
    setFolders(data || [])
  }
  ...
  async function loadFolderFiles(folderId) {
    ...
    const { data } = await supabase
      .from('files').select('*').eq('folder_id', folderId).order('name')
  ...
  async function handleDeleteFile(fileId, folderId) {
    ...
    if (file) await supabase.storage.from('course-files').remove([file.storage_path])
  ...
  async function handleDeleteFolder(folderId) {
    ...
    if (paths.length > 0) await supabase.storage.from('course-files').remove(paths)
    await supabase.from('file_folders').delete().eq('id', folderId)
    setFolders(folders.filter(f => f.id !== folderId))
    setFolderFiles(prev => { const next = { ...prev }; delete next[folderId]; return next })
  ...
  async function handleUpdatePermission(folderId, newLevel) {
    await supabase.from('file_folders')
      .update({ permission_level: parseInt(newLevel) }).eq('id', folderId)
    setFolders(folders.map(f =>
      f.id === folderId ? { ...f, permission_level: parseInt(newLevel) } : f
    ))
  }

  const permissionLabel = { 1: 'Admin + Tutor + Student', 2: 'Admin + Tutor', 3: 'Admin only' }
  ...
  return (
    <main className="min-h-screen bg-gray-100">
      ...
      <div className="px-8 py-8 grid grid-cols-1 md:grid-cols-2 gap-12">
        <div>
          <h3 className="text-sm font-black uppercase tracking-widest mb-6">Upload ZIP Archive</h3>
          <form onSubmit={handleUpload} className="flex flex-col gap-4">
            ...
            <div>
              <label className="text-xs text-gray-400 uppercase tracking-widest block mb-1">Permission Level</label>
            ...
            {result && (
              <div className="border border-black bg-white p-4 text-sm">
                ...
                <p className="text-gray-600">{result.folders_created} folder(s) created, {result.files_uploaded} file(s) uploaded</p>
                {result.errors?.length > 0 && (
                  <ul className="mt-2 text-red-600">{result.errors.map((e, i) => <li key={i}>{e}</li>)}</ul>
                )}
              </div>
            )}
        ...
        <div>
          <h3 className="text-sm font-black uppercase tracking-widest mb-6">Folders</h3>
          {folders.length === 0 ? (
                        ...
                        folderFiles[folder.id].map((file) => (
                          <div key={file.id}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button onClick={() => handleDeleteFile(file.id, folder.id)}
                                className="text-xs text-red-500 hover:text-red-700 font-bold">
                                DELETE
                              </button>
                            </div>
