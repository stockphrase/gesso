...
export default function CoursesPage() {
  ...
  async function loadCourses(userId) {
    ...
    setCourses(
      memberships
        ?.map((m) => ({ ...m.courses, memberRole: m.role }))
        .filter(Boolean) || [],
    );
  }
      ...
      <main className="p-8">
        ...
        {showForm && (
          <form
            ...
            className="border border-black p-6 mb-8 flex flex-col gap-4 max-w-lg"
          >
            <div>
            ...
            <div className="border border-gray-200 p-3 text-xs text-gray-400">
              ...
              <span className="font-black text-black">
                {profile?.name?.split(" ").pop() || "LastName"} |{" "}
                {courseName || "Course Name"} | {section || "00"} | {term} |{" "}
                {year}
              </span>
            </div>
