...
function TutorAssignmentsView({ profile, assignments, courseId }) {
  ...
  async function downloadSingle(sub) {
    ...
    try {
      const res = await fetch(
        `/api/submissions/admin-download?submission_id=${sub.id}`,
      );
  ...
  async function downloadReturnedZip(stage) {
    ...
    try {
      ...
      const zip = new JSZip();
      const returned = subs.filter(
        (s) => s.draft_stage === stage && s.returned_storage_path,
      );
      ...
      for (const sub of returned) {
        ...
        if (!student) continue;
        const res = await fetch(
          `/api/submissions/download-returned?submission_id=${sub.id}`,
        );
        const data = await res.json();
        ...
        const name =
          `${student.name}_${selected.title}_${stage}_returned.${ext}`.replace(
            /\s+/g,
            "_",
          );
        zip.file(name, blob);
      ...
      const a = document.createElement("a");
      a.href = blobUrl;
      a.download = `${selected.title}_${stage}_returned.zip`.replace(
        /\s+/g,
        "_",
      );
      a.click();
            ...
            <table className="w-full border-collapse border border-black text-xs font-bold tracking-widest uppercase">
              <thead>
                <tr className="border-b border-black">
                  <th className="text-left p-3 border-r border-black">
                    Student
                  </th>
                  {selected.draft_stages.map((stage) => (
                    <th
                      key={stage}
                      className="text-left p-3 border-r border-black last:border-r-0"
                    >
                      <div>{stage}</div>
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => downloadReturnedZip(stage)}
                  ...
                  >
                    ...
                    {selected.draft_stages.map((stage) => {
                      ...
                      return (
                        <td
                          key={stage}
                          className="p-3 border-r border-black last:border-r-0"
                        >
                          {!sub ? (
                            <div className="flex items-center gap-2">
                              <span className="w-3 h-3 rounded-full bg-red-500 shrink-0" />
                            ...
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-2">
                                <span
                                  className={`w-3 h-3 rounded-full shrink-0 ${sub.returned_at ? "bg-blue-500" : "bg-green-500"}`}
                                />
                                <button
          ...
          {assignments.map((a) => (
            ...
            >
              ...
              <p className="text-xs font-bold tracking-widest uppercase text-gray-400 group-hover:text-gray-300 shrink-0 ml-4">
                {a.draft_stages.length} stage
                {a.draft_stages.length !== 1 ? "s" : ""}
              </p>
            </button>
          ))}
        </div>
...
export default function AssignmentsPage() {
  ...
  async function downloadReturn(sub) {
    ...
    try {
      const res = await fetch(
        `/api/submissions/download?submission_id=${sub.id}`,
      );
      const data = await res.json();
      if (data.url) {
        const blob = await fetch(data.url).then((r) => r.blob());
                ...
                <table className="w-full border-collapse border border-black text-xs font-bold tracking-widest uppercase">
                  <thead>
                    <tr className="border-b border-black">
                      ...
                      {[
                        ...new Set(assignments.flatMap((a) => a.draft_stages)),
                      ].map((stage) => (
                        <th
                          key={stage}
                          className="text-left p-3 border-r border-black last:border-r-0"
                      ...
                      >
                        <td className="p-3 border-r border-black">
                          ...
                          {assignment.due_dates &&
                            Object.keys(assignment.due_dates).length > 0 && (
                              <div className="mt-2 flex flex-col gap-1">
                                {assignment.draft_stages.map((stage) =>
                                  assignment.due_dates[stage] ? (
                                    <p
                                      key={stage}
                                      className="text-gray-400 font-normal normal-case tracking-normal"
                                    >
                                      {stage}: due{" "}
                                      {new Date(
                                        assignment.due_dates[stage],
                        ...
                        {[
                          ...new Set(
                            assignments.flatMap((a) => a.draft_stages),
                          ),
                        ].map((stage) => {
                          ...
                          if (!isStage) {
                            return (
                              <td
                                key={stage}
                                className="p-3 border-r border-black last:border-r-0 bg-gray-50"
                              />
                            );
                          }
                            ...
                            >
                              {!sub ? (
                                <label className="flex items-center gap-2 cursor-pointer group">
                                  <span
                                    className={`w-3 h-3 rounded-full shrink-0 ${overdue ? "bg-orange-500" : "bg-red-500"}`}
                                  />
                                ...
                                <div className="flex flex-col gap-1">
                                  ...
                                  >
                                    ...
                                    <span className="font-mono text-gray-400 group-hover:underline">
